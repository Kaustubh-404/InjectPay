...
contract ElectraLiquidity is Ownable, ReentrancyGuard {
     ...
     */
    function withdrawFromLiquidityPool(string memory _positionId) external nonReentrant {
     ...
     * @param _positionId Position identifier
     */
    function harvestYield(string memory _positionId) external nonReentrant {
        Position storage position = positions[_positionId];
     ...
     * @param _positionId Position identifier
     */
    function addToPosition(string memory _positionId) external payable nonReentrant {
        if (msg.value == 0) revert InvalidAmount();
     ...
     */
    function calculatePendingRewards(string memory _positionId) public view returns (uint256 pendingRewards) {
        Position storage position = positions[_positionId];
